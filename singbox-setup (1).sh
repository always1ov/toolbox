...
phase_install() {
    ...
    local SB_VER
    SB_VER=$(wget -qO- "https://api.github.com/repos/SagerNet/sing-box/releases/latest"              | grep '"tag_name"' | grep -o 'v[0-9.]*' | head -1)

    if [ -z "$SB_VER" ]; then
        warn "无法获取最新版本号，使用固定版本 v1.13.11"
        SB_VER="v1.13.11"
    fi
    ...
    # 用镜像站下载 sing-box 二进制（多个镜像备用）
    local VER_NUM="${SB_VER#v}"
    local FILENAME="sing-box-${VER_NUM}-linux-${ARCH}.tar.gz"
    local MIRRORS=(
        "https://ghfast.top/https://github.com/SagerNet/sing-box/releases/download/${SB_VER}/${FILENAME}"
        "https://gh-proxy.com/https://github.com/SagerNet/sing-box/releases/download/${SB_VER}/${FILENAME}"
        "https://mirror.ghproxy.com/https://github.com/SagerNet/sing-box/releases/download/${SB_VER}/${FILENAME}"
        "https://github.com/SagerNet/sing-box/releases/download/${SB_VER}/${FILENAME}"
    )
    ...
    for mirror in "${MIRRORS[@]}"; do
        info "尝试下载：$mirror"
        if wget -t 2 -T 30 -q --show-progress -O "$TMPFILE" "$mirror" 2>/dev/null; then
            # 验证文件完整性
    ...
    # 用 233boy 脚本 + -f 参数本地安装（脚本本身也走镜像）
    info "下载 233boy 安装脚本..."
    local INSTALL_SCRIPT="/tmp/sb-install.sh"
    local SCRIPT_MIRRORS=(
        "https://ghfast.top/https://github.com/233boy/sing-box/raw/main/install.sh"
        "https://gh-proxy.com/https://github.com/233boy/sing-box/raw/main/install.sh"
        "https://mirror.ghproxy.com/https://github.com/233boy/sing-box/raw/main/install.sh"
        "https://github.com/233boy/sing-box/raw/main/install.sh"
    )

    local SCRIPT_OK=false
    for mirror in "${SCRIPT_MIRRORS[@]}"; do
        if wget -t 2 -T 20 -q -O "$INSTALL_SCRIPT" "$mirror" 2>/dev/null; then
    ...
    # 执行安装（-f 指定本地已下载的二进制文件）
    info "开始安装..."
    if bash "$INSTALL_SCRIPT" -f "$TMPFILE"; then
        ok "sing-box 安装完成"
        rm -f "$TMPFILE" "$INSTALL_SCRIPT"
    else
        err "sing-box 安装失败"
        rm -f "$TMPFILE" "$INSTALL_SCRIPT"
        exit 1
